...
void I2C_Read(uint8_t ADDR, uint8_t *i2cBif, uint8_t NofData)
{
	...
	MPUADDR = (MPU_ADDR<<1);
	i2cBuf[0] = ADDR;
	stat = HAL_I2C_Master_Transmit(&i2cHandler, MPUADDR, i2cBuf, 1, 10);
	if (stat != HAL_OK) {
		HAL_GPIO_TogglePin(GPIOC, 13);	// Error
		HAL_DELAY(100);
		HAL_GPIO_TogglePin(GPIOC, 13);	// Error
		HAL_DELAY(100);
	}
	HAL_DELAY(1);
	stat = HAL_I2C_Master_Receive(&i2cHandler, MPUADDR, i2cBif, NofData, 100);
	if (stat != HAL_OK) {
			HAL_GPIO_TogglePin(GPIOC, 13);	// Error
			HAL_DELAY(100);
			HAL_GPIO_TogglePin(GPIOC, 13);	// Error
...
void I2C_Write8(uint8_t ADDR, uint8_t data)
{
	...
	stat = HAL_I2C_Master_Transmit(&i2cHandler, MPUADDR, i2cData, 2,100);
	if (stat != HAL_OK) {
				HAL_GPIO_TogglePin(GPIOC, 13);	// Error
				HAL_DELAY(100);
				HAL_GPIO_TogglePin(GPIOC, 13);	// Error
